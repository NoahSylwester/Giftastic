  ...
  function updateGifs(keyword) {
    ...
    $.ajax({
      ...
      response.data.forEach((gif) => {
        arr.push($(`<div class="gif"><p class="gif-rating">Title: ${gif.title}<br />Rating: ${gif.rating}<br /><a href="${gif.source_post_url}">Source</a></p><img class="gif-image" alt="gif" src="${gif.images.downsized_still.url}" data-alt="${gif.images.downsized.url}"></div>`))
      });
